...
function expandRGBA(pixels, w, h) {
    var n = w * h;
...
function padPairs(pairs, target) {
    var need = target * 6;
...
function permuteBlocks(src, order) {
    var n = order.length;
...
function renderBlocks(pairs, cols) {
...
function extractBlocks(data, cols, rows, stride) {
...
function encodeMetaStrip(W, H, gridBuf, outW) {
...
function decodeMetaStrip(data, totalW) {
...
class PixelObfuscator {
    ...
    obfuscate(imageData) {
        ...
        return new ImageData(fullBuf, outW, outH + STRIP_H);
    }
    recover(imageData) {
        ...
        return new ImageData(outData, recW, recH);
    }
}
var MAX_SIZE = 1920;
var origImg = null;
...
function loadImageData(img, w, h) {
...
function loadImage(file) {
    ...
    reader.onload = function () {
        ...
        img.onload = function () {
            ...
            get('obfBtn').disabled = false;
            get('recBtn').disabled = false;
            get('dlObfBtn').disabled = true;
            get('dlRecBtn').disabled = true;
            get('obfCanvas').width = 0;
            get('obfCanvas').height = 0;
            get('recCanvas').width = 0;
            get('recCanvas').height = 0;
...
export function initGallery() {
    var obfBtn = get('obfBtn');
    var recBtn = get('recBtn');
    var dlObfBtn = get('dlObfBtn');
    var dlRecBtn = get('dlRecBtn');
    var obfCanvas = get('obfCanvas');
    var recCanvas = get('recCanvas');
